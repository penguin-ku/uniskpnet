...
#-----------------------------------------------------------------------------
module RubyExampleCreateLayOut
...
def self.run
  ...
  require_relative 'RubyExampleCreateLayOut'
  error = RubyExampleCreateLayOut.create_layout_doc(path)
  if error.nil?
    iterate_entities(model.entities, Geom::Transformation.new([0,0,0]))

    RubyExampleCreateLayOut.save_layout_doc
  else
    UI.messagebox(error)
  end
end
...
def self.iterate_entities(entities, transformation)
  entities.each { |entity|
    if entity.is_a?(Sketchup::DimensionLinear)
      ...
      RubyExampleCreateLayOut.add_linear_dimension(start_point.to_a,
                                               end_point.to_a,
      ...
      RubyExampleCreateLayOut.add_label(text, vec.to_a, point.to_a, point_pid.to_s)
