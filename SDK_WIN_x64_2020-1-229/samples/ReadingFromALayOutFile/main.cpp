...
int main(int argc, const char* argv[]) {
  ...
  if (argc < 2) {
    std::cout << "Example: ReadingFromALayOutFile <filename.layout>" << std::endl;
  ...
  SUResult result = LODocumentRelease(&layout_doc_ref);
  
  if (result != SU_ERROR_NONE) {
    std::cout << "Reading LayOut File: Failed" << std::endl;
  } else {
    std::cout << "Reading LayOut File: Success" << std::endl;
  }
