...
namespace UnityEngine.TaskExtension
{
    ...
    public abstract class UnityTask
    {
        ...
        protected System.AggregateException _exception;
        ...
        /// 获取应用程序执行过程中发生的错误
        /// </summary>
        public System.AggregateException Exception => _exception;
        ...
        private static Dictionary<string, bool> g_loopDic = new Dictionary<string, bool>();
        private static object g_loopDicLock = new object();

        /// <summary>
        /// 取消指定循环
        /// </summary>
        /// <param name="p_loopIdentity"></param>
        public static void CancelLoop(string p_loopIdentity)
        {
            lock (g_loopDicLock)
            {
                g_loopDic[p_loopIdentity] = false;
            }
        }
        ...
        public static void CancelLoop()
        {
            lock (g_loopDicLock)
            {
                foreach (var item in g_loopDic.Keys)
                {
                    g_loopDic[item] = false;
                }
            }
        }
        ...
        /// <param name="count">循环次数，-1表示无限循环</param>
        /// <returns></returns>
        public static string Loop(Action action, int interval, int delay = 0, int count = -1)
        {
